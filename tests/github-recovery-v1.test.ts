...
test("build modes and approved public work remain separate", () => {
  ...
  // Canonical PUBLIC93 destination is retained; the recovery BETA manifest remains safety-only.
  assert.equal(JSON.parse(read(".openai/hosting.json").toString()).project_id, "appgprj_6a6dde04153c8191b9b8551a9f22db92");
  assert.equal(JSON.parse(read(".openai/hosting.public.json").toString()).project_id, "appgprj_6a6dde04153c8191b9b8551a9f22db92");

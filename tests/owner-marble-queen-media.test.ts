...
test('Marble Queen publishes the three Owner photos through the existing gallery contract', () => {
 ...
 assert.ok(gallery[0].src.includes('plante-tibaldo'));
 assert.ok(gallery[1].src.includes('detail-panachure-tibaldo'));
 assert.ok(gallery[2].src.includes('vue-trois-quarts-tibaldo'));

...
const manifest = JSON.parse(readFileSync(new URL('../docs/media-provenance/photo-20260913-four-01.json', import.meta.url), 'utf8'));
for (const photo of manifest.photos) {
  const [_, genre, slug] = photo.canonical_slug.split('/');
  test(`${slug}: canonical primary, genus and API use the exact Owner WebP`, () => {
    ...
    assert.equal(getPlantsByGenre(genre).find(p => p.slug === slug)?.gallery[0].src, media.src);

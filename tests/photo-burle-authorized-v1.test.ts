...
const manifest = JSON.parse(readFileSync(new URL('../docs/media-provenance/photo-20260910-burle-01.json', import.meta.url), 'utf8'));
for (const photo of manifest.photos) {
  ...
  test(`${slug}: canonical primary, genus and API use the exact Owner WebP`, () => {
    ...
    assert.equal(media.caption, 'Photo : Rare Plant Fairy');
    assert.equal(media.license?.creator, 'Rare Plant Fairy');
    ...
    assert.equal(api.primary_media_creator, 'Rare Plant Fairy');
  });
}

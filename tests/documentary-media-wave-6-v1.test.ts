...
test("Wave 6 plus Owner-authorized media expose 78 verified identities", () => {
...
test("Owner photo for Monstera Mint is exact, verified, and locally present", () => {
  const { api, image } = contract("monstera", "mint");
  assert.equal(api.media_status, "VERIFIED_MEDIA");
  assert.match(api.primary_media_url, /owner-media\/monstera\/monstera-mint-owner-2026-09\.webp$/);
  assert.equal(image?.width, 1080);
  assert.equal(image?.height, 1350);
  assert.match(image?.alt ?? "", /Monstera.*Mint/i);
  assert.ok(existsSync(new URL(`../public${image?.src}`, import.meta.url)));
});

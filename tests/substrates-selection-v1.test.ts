...
test("product source registry records bounded partner use; no runtime supplier fetch", () => {
  for (const entry of sourceRegistry) {
    ...
    assert.equal(entry.reviewedAt, "2026-09-03"); assert.equal(entry.mediaRights, "OWNER_CONFIRMED_PARTNERSHIP_PUBLIC_USE");
  }
...
test("only two authorized press photographs, exact bytes and local delivery", () => {
  ...
  assert.equal(supplierMediaRights.packshotRights,"OWNER_CONFIRMED_PARTNERSHIP_PUBLIC_USE");

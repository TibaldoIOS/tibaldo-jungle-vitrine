...
test("verified media API classifies all 96 canonical entries without fuzzy association", () => {
  ...
  assert.equal(contracts.filter(({ media_status }) => media_status === "VERIFIED_MEDIA").length, 79);
  assert.equal(contracts.filter(({ media_status }) => media_status === "HONEST_MEDIA_GAP").length, 17);

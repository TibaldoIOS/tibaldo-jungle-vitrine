...
export default async function Page({ params }: Props) {
  ...
  return <main className="editorial-page"><ScrollReveal />
    ...
    {genre === "strelitzia" ? <PlantGenusHero genre={genre} label="Genre végétal" title="Oiseaux de paradis" titleLead="Strelitzia" subtitle={guide.heroSubtitle} isFamily={isFamily} /> : <section className="inner-hero compact-inner-hero family-genre-hero">{herbier && <div className="family-genre-image is-herbier" role="img" aria-label={herbier.alt} style={{ backgroundImage: `url(${herbier.image})`, backgroundPosition: herbier.position }} />}<div className="inner-hero-shade" /><SiteHeader /><div className="shell inner-hero-content"><a className="family-genre-breadcrumb" href="/plantes">Encyclopédie <span>·</span> Tous les univers</a><p className="eyebrow"><span /> {isFamily ? "Famille botanique" : "Genre végétal"}</p><h1><span className="hero-line"><span>Les</span></span><span className="hero-line"><span><em>{isFamily ? botanicalName : guide.name}.</em></span></span></h1><p>{guide.heroSubtitle}</p></div></section>}

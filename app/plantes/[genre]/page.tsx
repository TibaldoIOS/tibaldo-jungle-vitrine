...
const geoRevisedGenres = new Set(["strelitzia", "alocasia"]);
...
export default async function Page({ params }: Props) {
  ...
  const schema = {
    ...
    "@graph": [
      { "@type": "Article", headline: `${guide.name} : guide de culture, espèces et variétés`, description: guide.lead, image: guide.image, ...(geoRevisedGenres.has(genre) ? { dateModified: "2026-08-21" } : {}), author: { "@type": "Organization", name: "Studio Végétal – Tibaldo Jungle", url: "https://jungle.tibaldo.fr/methodologie-sources" }, publisher: { "@type": "Organization", name: "Studio Végétal – Tibaldo Jungle", alternateName: "Tibaldo Jungle", url: "https://jungle.tibaldo.fr" } },
  ...
  return <main className="editorial-page"><ScrollReveal />
    ...
    <article className="family-guide"><section className="shell family-guide-intro" data-reveal><div><p className="section-kicker">Guide de culture · Lille</p><h2>Comprendre<br /><em>les {guide.name}.</em></h2></div><div><p>{guide.lead}</p><p>{guide.origin}</p>{geoRevisedGenres.has(genre) && <p className="editorial-date">Révisé le <time dateTime="2026-08-21">21 août 2026</time> · <Link href="/methodologie-sources">Méthode éditoriale</Link></p>}</div></section><section className="family-editorial shell">{editorials.map((section, index) => <article key={section.title} data-reveal><span>{String(index + 1).padStart(2, "0")}</span><h2>{section.title}</h2><p>{section.text}</p></article>)}</section><section className="family-guide-facts shell">{guide.facts.map((fact) => <div key={fact.label} data-reveal><span>{fact.label}</span><strong>{fact.value}</strong></div>)}</section>
    {genre === "strelitzia" && <><section className="shell geo-answer" data-reveal><p className="section-kicker">Réponse directe</p><h2>Strelitzia alba ou nicolai :<br /><em>quelles différences ?</em></h2><p><strong>Les deux sont de grands Strelitzia arborescents à fleurs blanches, difficiles à distinguer sur une feuille seule.</strong> Strelitzia nicolai est très courant en culture et peut porter plusieurs spathes bleu sombre ; S. alba est une espèce du Cap dont Strelitzia augusta est un synonyme botanique selon Kew. Pour identifier un sujet, croisez architecture adulte, inflorescence, provenance et étiquette documentée.</p><div><article><h3>Le plus facile en intérieur</h3><p>S. nicolai est le choix le plus courant et le mieux documenté en intérieur, à condition de disposer d’une lumière très forte et d’un grand volume. « Facile » ne signifie pas adapté à un fond de pièce.</p></article><article><h3>Feuilles fendues</h3><p>Les grands limbes se fendent souvent sous l’effet du vent et des contraintes mécaniques. Ce phénomène n’indique pas à lui seul une maladie ; protégez surtout la plante des passages et courants violents.</p></article></div><p className="geo-source-note">Faits taxonomiques vérifiés avec Kew — Plants of the World Online ; conseils horticoles à adapter aux conditions de culture.</p></section><section className="shell strelitzia-comparison" data-reveal><p className="section-kicker">Comparer les cinq espèces</p><h2>Des oiseaux de paradis<br /><em>vraiment différents.</em></h2><div className="strelitzia-comparison-scroll"><table><thead><tr><th>Espèce</th><th>Port</th><th>Feuillage</th><th>Floraison</th><th>Culture</th><th>Particularité</th></tr></thead><tbody><tr><th>reginae</th><td>Touffe compacte</td><td>Limbe large</td><td>Orange et bleu</td><td>Intérieur très lumineux, pot</td><td>La plus florifère en culture</td></tr><tr><th>juncea</th><td>Touffe verticale</td><td>Limbe très réduit à maturité</td><td>Orange et bleu</td><td>Soleil, pot drainant</td><td>Silhouette de jonc</td></tr><tr><th>nicolai</th><td>Arborescent</td><td>Très grandes feuilles</td><td>Blanche et bleu sombre</td><td>Grand intérieur, extérieur estival</td><td>Très courant dans le commerce</td></tr><tr><th>alba</th><td>Arborescent</td><td>Grand éventail</td><td>Blanche</td><td>Grand volume, hors gel</td><td>Augusta est son synonyme botanique</td></tr><tr><th>caudata</th><td>Arborescent montagnard</td><td>Grandes feuilles</td><td>Pâle</td><td>Collection, hors gel prudent</td><td>Rare en culture</td></tr></tbody></table></div></section></>}
    {genre === "alocasia" && <section className="shell geo-answer" data-reveal><p className="section-kicker">Réponse directe</p><h2>Alocasia ou Colocasia :<br /><em>comment les distinguer ?</em></h2><p><strong>Ce sont deux genres distincts de la famille des Araceae.</strong> Beaucoup d’Alocasia portent des feuilles plutôt dressées, dont le pétiole rejoint le limbe près du sinus basal ; chez de nombreux Colocasia, l’insertion est plus peltée et les feuilles s’orientent davantage vers l’extérieur ou le bas. Ces repères ne remplacent pas l’identification de l’espèce.</p><div><article><h3>Culture</h3><p>Les Alocasia apprécient généralement un substrat aéré, chaud et régulièrement humide sans saturation. Les Colocasia cultivés activement tolèrent souvent davantage d’eau et une nutrition généreuse.</p></article><article><h3>Repos et hiver</h3><p>Selon l’espèce, la température et la lumière, les deux genres peuvent ralentir ou perdre du feuillage. Évitez toute règle absolue : identifiez le taxon et adaptez eau, chaleur et hivernage.</p></article></div><Link href="/plantes/colocasia">Découvrir le guide Colocasia →</Link></section>}

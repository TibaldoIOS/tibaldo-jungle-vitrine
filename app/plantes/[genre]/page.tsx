...
export default async function Page({ params }: Props) {
  ...
  return <main className="editorial-page"><ScrollReveal />
    ...
    <nav className="plant-explorer is-compact" aria-label={`Espèces et variétés de ${guide.name}`}><div className="shell plant-explorer-inner"><div><span>Explorer les espèces & variétés</span><div className="plant-explorer-links">{list.length ? list.map((plant) => <a href={`/plantes/${genre}/${plant.slug}`} key={plant.slug}>{plant.botanicalName}</a>) : <span className="is-disabled">Premières fiches à venir</span>}</div></div></div></nav>

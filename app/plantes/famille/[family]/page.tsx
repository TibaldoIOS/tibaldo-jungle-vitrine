...
export default async function PlantFamilyPage({ params }: Props) {
  ...
  return <main className="editorial-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} /><ScrollReveal /><section className="inner-hero compact-inner-hero"><div className="inner-hero-texture" /><div className="inner-hero-shade" /><SiteHeader /><div className="shell inner-hero-content"><Link className="family-genre-breadcrumb" href="/plantes">Encyclopédie <span>·</span> Familles botaniques</Link><p className="eyebrow"><span /> Famille botanique</p><h1><span className="hero-line"><span>Les</span></span><span className="hero-line"><span><em>{family}.</em></span></span></h1><p>{genera.join(" · ")} : découvrez les genres, espèces et cultivars documentés dans notre bibliothèque végétale.</p></div></section><section className="plant-index shell"><div data-reveal><p className="section-kicker">Classification botanique</p><h2>{family}.<br />Une famille, plusieurs écritures du vivant.</h2><p>Cette page rassemble volontairement les fiches de la famille {family}. Elle s’enrichira automatiquement à mesure que de nouvelles plantes seront documentées.</p></div><div className="plant-index-grid">{matches.map((plant) => <Link href={`/plantes/${plant.genre}/${plant.slug}`} key={plant.slug} data-reveal><Image unoptimized src={plant.gallery[0].src} alt={publicPlantImageAlt(plant.gallery[0].src, plant.botanicalName, plant.gallery[0].alt)} width={plant.gallery[0].width} height={plant.gallery[0].height} /><span>{plant.taxonomy.genus} · {plant.taxonomy.species}</span><h2>{plant.botanicalName}</h2><p>{plant.subtitle}</p><strong>Lire la fiche <Arrow /></strong></Link>)}</div></section><SiteFooter /></main>;
}

...
export default function PlantFamilyDirectory({ family, plants }: { family: string; plants: readonly PlantEntry[] }) {
  ...
  return <main className={`${styles.page} editorial-page`} data-golden-family-v25={family.toLowerCase()}><ScrollReveal />
    <section className={styles.hero}><SiteHeader /><div className={`${styles.heroContent} shell`}><Link href="/plantes">Encyclopédie · Familles botaniques</Link><h1>Les <em>{family}.</em></h1><p>{genera.join(" · ")} : une famille, plusieurs écritures du vivant.</p></div></section>
    <section className={`${styles.intro} shell`} data-reveal><p className="section-kicker">Classification botanique</p><div><h2><VarietyCount count={plants.length} /> documentées.<br /><em>Un répertoire compact.</em></h2><p>Cette page rassemble les espèces et cultivars de la famille {family}. Elle privilégie la lecture et les liens sémantiques sans répéter une grande carte pour chaque taxon.</p></div></section>

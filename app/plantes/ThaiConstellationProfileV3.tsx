...
export default function ThaiConstellationProfileV3({ plant, gallery }: { plant: PlantEntry; gallery: PlantEntry["gallery"] }) {
  ...
  return <>
    <div className="plant-profile-layout thai-v3-layout shell"><aside><PlantSectionNav /></aside><div className="plant-profile-content thai-v3-content">
      ...
      <section className="plant-profile-section thai-v3-neighbours"><header data-reveal><p className="section-kicker">Poursuivre l’exploration</p><h2>D’autres Monstera,<br/><em>d’autres silhouettes.</em></h2></header><div>{neighbours.map((item, index) => <Link href={`/plantes/monstera/${item.slug}`} key={item.slug} data-reveal><img src={item.gallery[0].src} alt={item.gallery[0].alt} width={item.gallery[0].width} height={item.gallery[0].height} loading="lazy"/><span>0{index + 1}</span><strong>{item.displayName}</strong><small>{item.subtitle}</small><Arrow /></Link>)}</div><Link className="thai-v3-back-genus" href="/plantes/monstera">Voir tout le genre Monstera <Arrow /></Link></section>

      <section className="plant-gallery plant-profile-section thai-v3-gallery" data-reveal><p className="section-kicker">Galerie végétale</p><div>{gallery.map((image) => <figure key={image.src}><img src={image.src} alt={image.alt} width={image.width} height={image.height} loading="lazy"/><figcaption>{image.caption}</figcaption></figure>)}</div></section>

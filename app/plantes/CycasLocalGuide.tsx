  ...
  return (
    <section className={styles.section} aria-labelledby="cycas-lille-title" data-reveal>
      ...
      <div className={styles.detail}>
        <article><h3>Pot ou pleine terre&nbsp;?</h3><p>Le pot permet de déplacer la plante avant une période froide prolongée. Une plantation en pleine terre demande un emplacement exceptionnellement abrité, un sol très drainant et ne permet jamais de garantir la survie lors d’un hiver défavorable.</p></article>
        <article><h3>Quand le mettre à l’abri&nbsp;?</h3><p>Ne raisonnez pas sur une température isolée. Anticipez une séquence de gel, un substrat qui reste humide ou des vents froids, puis placez le sujet dans un espace lumineux, ventilé et hors gel. Réduisez l’arrosage sans laisser les racines se dessécher totalement.</p></article>
        <article><h3>Retour dehors au printemps</h3><p>Réhabituez progressivement les frondes au soleil sur plusieurs semaines. Une sortie brutale peut brûler un feuillage formé sous abri ; attendez aussi que le mélange sèche à un rythme régulier avant de reprendre les arrosages de croissance.</p></article>
      </div>
      ...
      <div className={styles.sources} aria-label="Sources de la section Cycas revoluta">
        ...
        {plant.sources.map((source) => <a href={source.url} target="_blank" rel="noreferrer" key={source.url}>{source.label} <Arrow /></a>)}
      </div>
    </section>
  );
}

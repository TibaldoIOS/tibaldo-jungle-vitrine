...
import FlowerQuoteForm from "../fleurs/FlowerQuoteForm";
...
const faqs = [
  { question: "Puis-je acheter des fleurs coupées directement en boutique ?", answer: "Non. Les fleurs coupées sont proposées exclusivement sur commande afin de garantir une sélection fraîche et commandée spécialement auprès de fournisseurs professionnels." },
  { question: "Tibaldo Jungle compose-t-il les bouquets ?", answer: "Non. Tibaldo Jungle ne réalise pas de composition florale artisanale. Nous pouvons fournir des fleurs coupées en bottes ou rechercher des bouquets déjà confectionnés par nos fournisseurs professionnels, selon leurs disponibilités." },
...
export default function FlowersOnOrderPage() {
  ...
  return <main className="editorial-page flowers-page"><ScrollReveal /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} />
    ...
    <section className="shell flowers-intro"><header data-reveal><p className="section-kicker">Une offre simple et transparente</p><h2>Vous imaginez.<br /><em>Nous organisons la commande.</em></h2></header><div data-reveal><p>Tibaldo Jungle vous donne accès à une sélection de fleurs coupées issue de fournisseurs professionnels. Vous nous confiez votre date, vos envies, votre palette de couleurs et les quantités souhaitées : nous vérifions les possibilités, préparons le chiffrage puis lançons l’approvisionnement après validation.</p><p>Le Studio ne compose pas lui-même de bouquets et ne se présente pas comme artisan fleuriste. Nous fournissons des bottes de fleurs coupées, des feuillages ou, sur demande, des bouquets déjà confectionnés par un fournisseur professionnel. Cette formule évite de facturer une prestation de composition Tibaldo Jungle ; le devis reste transparent et comprend l’achat, l’organisation, la manutention et l’éventuelle livraison.</p><strong>Les fleurs et bouquets présentés restent soumis aux disponibilités, aux saisons, aux arrivages et aux tarifs du fournisseur.</strong></div></section>
    ...
    <nav className="shell flower-service-link" aria-label="Découvrir les offres florales" data-reveal><Link href="/fleurs-mariage-lille">Fleurs coupées pour un mariage à Lille <span>↗</span></Link><Link href="/fleurs-evenement-lille">Fleurs coupées pour un événement à Lille <span>↗</span></Link></nav>
    <section className="flowers-notice"><div className="shell" data-reveal><p>Important</p><h2>Uniquement sur commande.</h2><span>Pas de vente libre de fleurs coupées en magasin. Un délai raisonnable est nécessaire pour consulter les disponibilités, établir le devis et organiser l’approvisionnement.</span><a className="button button-light" href="#devis-fleurs">Parler de votre projet <Arrow /></a></div></section>
    <section className="shell flower-local-form" id="devis-fleurs"><div><p className="section-kicker">Demande de devis</p><h2>Décrivez votre<br /><em>projet floral.</em></h2></div><FlowerQuoteForm /></section>

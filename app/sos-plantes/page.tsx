...
export default function Page() {
  ...
  return <main className="editorial-page sos-page">
    ...
    <section className="sos-signs" aria-labelledby="sos-signs-title"><div className="shell"><header data-reveal><p className="section-kicker">Premiers indices</p><h2 id="sos-signs-title">Ce que la plante<br/><em>vous montre.</em></h2></header><div className="sos-sign-grid">{signs.map((sign, index) => <article key={sign.title} data-reveal><Image unoptimized src={sign.image} alt={sign.alt}/><span>{String(index + 1).padStart(2, "0")} · {sign.label}</span><h3>{sign.title}</h3><p>{sign.copy}</p></article>)}</div></div></section>
    ...
    <section className="shell sos-service-paths"><article data-reveal><span>SOS Plantes</span><h2>Comprendre un problème.</h2><p>Diagnostic, parasites, feuilles abîmées ou croissance anormale : nous cherchons d’abord la cause.</p><Link href="/sos-plantes#diagnostic">Faire examiner ma plante <Arrow /></Link></article><article data-reveal><span>Bar à rempotage</span><h2>Intervenir sur le pot.</h2><p>Changement de contenant, mélange adapté et travail des racines : un service distinct quand il est réellement nécessaire.</p><Link href="/rempotage-plantes-lille">Découvrir le rempotage <Arrow /></Link></article></section>

    <section className="local-topic-links shell"><Link href="/sos-plantes#diagnostic"><span>Examen au Studio</span><strong>Faire diagnostiquer une plante à Lille</strong></Link><Link href="/conseils/thrips-plantes-interieur"><span>Parasites</span><strong>Reconnaître les thrips sur vos plantes</strong></Link><Link href="/rempotage-plantes-lille"><span>Autre univers</span><strong>Découvrir le Bar à rempotage</strong></Link></section>

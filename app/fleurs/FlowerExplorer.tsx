...
export default function FlowerExplorer({ flowers }: { flowers: FlowerEntry[] }) {
  ...
  const selectedFlowers = flowers.filter((flower) => selection.includes(flower.slug));
  ...
  return <section className="flower-explorer shell" aria-labelledby="flower-explorer-title">
    ...
    <div className="flower-grid">{results.map((flower, index) => <article key={flower.slug} className={selection.includes(flower.slug) ? "is-selected" : ""} data-reveal><div className={`flower-art flower-art-${index % 6}`} aria-hidden="true"><span>{flower.name.slice(0, 1)}</span></div><div className="flower-card-copy"><span>{flower.family} · {flower.category}</span><h3>{flower.name}</h3><i>{flower.botanicalName}</i><p>{flower.description}</p><ul><li>{flower.seasons.join(" · ")}</li><li>Tenue indicative : {flower.vaseLife}</li></ul>{flower.faq && <a className="flower-card-detail" href={`/fleurs/${flower.slug}`}>Découvrir la fiche ↗</a>}<button type="button" onClick={() => toggle(flower.slug)}>{selection.includes(flower.slug) ? "Retirer de mon projet ✓" : "Ajouter à mon projet +"}</button></div></article>)}</div>
    <aside className={`flower-selection${selection.length ? " is-visible" : ""}`} aria-live="polite"><div><span>Votre sélection d’inspiration</span><strong>{selection.length} {selection.length > 1 ? "fleurs" : "fleur"}</strong></div><a href="#demande-devis" data-action="quote">Préparer mon devis <span>↓</span></a></aside>
    <section className="flower-explorer-quote" id="demande-devis"><header><p className="section-kicker">Projet floral</p><h2>Transformer l’inspiration<br /><em>en demande concrète.</em></h2><p>Votre sélection sera jointe à la demande. Aucun achat n’est déclenché avant votre validation du devis.</p><ol className="flower-order-steps"><li><span>01</span>Vous décrivez le projet</li><li><span>02</span>Nous vérifions les arrivages</li><li><span>03</span>Vous validez le devis</li><li><span>04</span>L’acompte confirme la commande</li></ol>{selectedFlowers.length > 0 && <div className="flower-selected-summary"><strong>Votre sélection</strong><div>{selectedFlowers.map((flower) => <button type="button" key={flower.slug} onClick={() => toggle(flower.slug)}>{flower.name}<span>×</span></button>)}</div></div>}</header><FlowerQuoteForm initialFlowers={selection} /></section>
  </section>;
}

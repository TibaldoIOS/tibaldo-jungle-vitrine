...
export default function Page() {
  ...
  return <main className="editorial-page guides-page">
    ...
    <section className="advice-featured"><div className="shell"><header data-reveal><p className="section-kicker">À lire maintenant</p><h2>Les questions qui<br/><em>reviennent au Studio.</em></h2></header><div className="advice-featured-grid">{featured.map((guide, index) => <Link href={`/conseils/${guide.slug}`} key={guide.slug} className={index === 0 ? "is-main" : ""} data-reveal><Image unoptimized src={guide.image} alt=""/><div><span>{guide.eyebrow} · {guide.readingTime}</span><h3>{guide.title}</h3><p>{guide.intro}</p><strong>Lire le guide ↗</strong></div></Link>)}</div></div></section>

...
const sections = [
  ...
  ["Responsabilité et correction", `Le contenu est édité par ${jungleLocalIdentity.storeName}, ${jungleLocalIdentity.streetAddress} à ${jungleLocalIdentity.city}. Une erreur factuelle ou une source devenue obsolète peut être signalée à ${jungleLocalIdentity.email} ; elle sera vérifiée avant correction.`],
] as const;

export default function MethodologieSourcesPage() {
  const jsonLd = { "@context": "https://schema.org", "@graph": [
    { "@type": "Article", "@id": `${canonical}#article`, headline: "Méthodologie éditoriale et sources", description: metadata.description, datePublished: "2026-08-21", dateModified: "2026-09-02", mainEntityOfPage: canonical, author: { "@id": jungleLocalIdentity.organizationId }, publisher: { "@id": jungleLocalIdentity.organizationId } },
  ...
  return <main className="editorial-page methodology-page">
    ...
    <article className="shell methodology-content"><header><p className="section-kicker">Responsabilité éditoriale</p><h2>Des informations vérifiables,<br /><em>des conseils toujours contextualisés.</em></h2><p>Publié le <time dateTime="2026-08-21">21 août 2026</time> · révisé le <time dateTime="2026-09-02">2 septembre 2026</time>.</p></header>{sections.map(([title, text], index) => <section key={title}><span>{String(index + 1).padStart(2, "0")}</span><div><h2>{title}</h2><p>{text}</p></div></section>)}<aside><h2>Références privilégiées</h2><p><a href="https://powo.science.kew.org/" target="_blank" rel="noreferrer">Kew — Plants of the World Online</a> · <a href="https://www.worldfloraonline.org/" target="_blank" rel="noreferrer">World Flora Online</a> · <a href="https://www.rhs.org.uk/plants" target="_blank" rel="noreferrer">Royal Horticultural Society</a></p></aside></article>
    <SiteFooter />
  </main>;
}
